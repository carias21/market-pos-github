...
class VentasModelo
{
    ...
    static public function mdlRegistrarVenta($datos)
    {
        ...
        try {
            ...
            foreach ($listaProductos as $producto) {
                ...
                $stmt->bindParam(":cantidad", $producto[3], PDO::PARAM_INT);
                ...
                $stmtCaja->bindParam(":cantidad", $producto[3], PDO::PARAM_INT);
            ...
            $conn->rollBack();
            $resultado = "ERROR REGISTRO VENTA: " . $e->getMessage();
        } catch (Exception $e) {
            $conn->rollBack();
            ...
            if (isset($stmtCaja)) $stmtCaja = null;
            if (isset($stmtStock)) $stmtStock = null;
    ...
    public static function verificarStock($conn, $producto, $codigo_producto)
    {
        try {
            ...
            if ($resultadoStock && ($resultadoStock['stock_producto'] == 0)) {
                $_POST["stock0"] = "ALERTA: EL PRODUCTO $producto CON CÓDIGO: $codigo_producto SE HA AGOTADO";
                include_once '../vistas/enviar_correo.php';

                return $_POST["stock0"];
            } elseif ($resultadoStock && ($resultadoStock['stock_producto'] == $resultadoStock['minimo_stock_producto'])) {
